...
def compute_performance_statistics(results: List[Dict], total_duration: float) -> Dict:
    ...
    :param results: List of result dictionaries from concurrent requests
    :param total_duration: time for all requests in seconds
    :return:
    """
...
def run_single_user_prediction(user_id: int) -> Dict:
    """
    Function to be run by each concurrent thread.
    Simulates one user making a request.
    """
...
def test_concurrent_predictions_sync():
    """
